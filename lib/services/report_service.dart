...
class ReportService {
  ...
  Future<List<Student>> fetchStudentsForSelects() async {
    final response = await _apiClient.get('/UsersForGroups/GetUsersForSelects', query: {'userTypeId': UserType.student.id});
    final container = (response['data'] ?? response['result'] ?? response) as Map<String, dynamic>;
    final rawItems = container['items'] as List<dynamic>? ?? const [];
    return rawItems.map((item) {
      final map = _normalize(item);
